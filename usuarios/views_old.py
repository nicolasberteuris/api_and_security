...
from .forms import Categoria_UsuarioForm, FormularioLogin, FormularioUsuario
...
class ListadoUsuario(ValidarPermisosRequeridosMixin,generic.ListView):
    template_name = 'usuarios/listar_usuarios.html'
    ...
    def get_context_data(self, **kwargs):
        context = super(ListadoUsuario, self).get_context_data(**kwargs)
        context.update({
...
class ModalRegistrarUsuario2(CreateView):
    model = Usuario
    template_name = 'usuarios/crear_usuario.html'
    #permission_required = ('usuarios.view_usuario','usuarios.add_usuario','usuarios.delete_usuario','usuarios.change_usuario')
    form_class = FormularioUsuario
    success_url = reverse_lazy ('dashboard:listado_productos')


#Categoria de usuario

class ModalRegistrarCategoria_Usuario(ValidarPermisosRequeridosMixin,CreateView):
    model = Categoria_Usuario
    template_name = 'usuarios/crear__categoria_usuario.html'
    permission_required = ('usuarios.view_categoria_usuario','usuarios.add_categoria_usuario','usuarios.delete_categoria_usuario','usuarios.change_categoria_usuario')
    form_class = Categoria_UsuarioForm
    success_url = reverse_lazy ('usuarios:listar_categoria_usuarios')

class EditarCategoria_Usuario(ValidarPermisosRequeridosMixin,UpdateView):
    model = Categoria_Usuario
    template_name = 'usuarios/editar_categoria_usuarios.html'
    permission_required = ('usuarios.view_categoria_usuario','usuarios.add_categoria_usuario','usuarios.delete_categoria_usuario','usuarios.change_categoria_usuario')
    form_class = Categoria_UsuarioForm
    success_url = reverse_lazy ('usuarios:listar_categoria_usuarios')

class EliminarCategoria_Usuario(ValidarPermisosRequeridosMixin,DeleteView):
    model = Categoria_Usuario
    permission_required = ('usuarios.view_categoria_usuario','usuarios.add_categoria_usuario','usuarios.delete_categoria_usuario','usuarios.change_categoria_usuario')
    success_url = reverse_lazy ('usuarios:listar_categoria_usuarios')

class ListadoCategoria_Usuario(ValidarPermisosRequeridosMixin,generic.ListView):
    template_name = 'usuarios/listar_categoria_usuarios.html'
    permission_required = ('usuarios.view_categoria_usuario','usuarios.add_categoria_usuario','usuarios.delete_categoria_usuario','usuarios.change_categoria_usuario')
    context_object_name = 'categoria_usuarios'
    model = Categoria_Usuario

    def get_context_data(self, **kwargs):
        context = super(ListadoCategoria_Usuario, self).get_context_data(**kwargs)
        context.update({
            'categoria_usuarios': Categoria_Usuario.objects.all(),
            #'categories': Category.objects.all(),
        })
        return context

    def get_queryset(self):
        return Categoria_Usuario.objects.all()